<script lang="ts">
  ...
  const canReadPeople = $derived(hasPermissions(asset, SharingPermission.PersonRead));
  const canEditPeople = $derived(hasPermissions(asset, SharingPermission.PersonUpdate));
...
{#if !authManager.isSharedLink && canReadPeople}
